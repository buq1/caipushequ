...
<div class="bj1">
<p style="margin-left:10px;line-height: 30px;"><i style="color: orange;" class="fa fa-diamond" aria-hidden="true"></i>&nbsp;<font style="color: brown;font-weight: bold;">早餐营养满满</font></p>
</div>
<div class="row">
    <?php
    $sql=mysqli_query($conn,"select count(*) as total from tb_caipu where leibie='早餐' ");
    $info=mysqli_fetch_array($sql);
    $total=$info['total'];
    if($total==0)
    {
        echo "<div style='width:100%;height:40px;text-align:center'>好可惜啊，没有早餐菜谱可以分享呢!</div>";
    }
    else
    {
    $pagesize=18;
    ...
    $sql1=mysqli_query($conn,"select * from tb_caipu where leibie='早餐' order by id desc limit ".($page-1)*$pagesize.",$pagesize ");
